...
$conex->set_charset("utf8mb4");
    ...
    $sql = "SELECT id_usuario, nombreusu, nombre, contrasena, es_admin FROM usuarios WHERE nombreusu = ?";
    ...
    $stmt->execute();
    $stmt->bind_result($id_usuario, $nombreusu, $nombre, $hashedPassword, $es_admin);
    $stmt->fetch();
...
if ($id_usuario && password_verify($pas, $hashedPassword)) {
    ...
    if ($es_admin) {
        ...
        header("Location: ../home/mensajeria.php"); // Redirigir al dashboard del administrador
    } else {
        // Si no es admin, redirigir a la página común
        header("Location: ../home/mensajeria.php"); // Redirigir a la página común
    }
    exit();
}
else {
